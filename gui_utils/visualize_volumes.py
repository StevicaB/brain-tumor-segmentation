...
class VisualVolumes(MyFrame):
    ...
    def __init__(self,
        ...
        self.set_title('Visualize Volumes')
    ...
    def set_variables(self,
        ...
        # Setting the initial axis.
        self._axis = 0

        # Variable for displaying supervoxel borders.
    ...
    def add_slice_bar(self, slice_frame):
        ...
        self.image_scale = (self.screen_height - 200) / 2
    ...
    def update_slice_scroll(self):
        """Updates the slice scroll of the images when new axis is shown."""

        # Getting the min and max value of the slices in the new axis.
        max_slice = self._get_max_slice()
        min_slice = self._get_min_slice()

        # Calculating the number of slices and putting the slider in the middle.
        diff = max_slice - min_slice
        new_value = int(min_slice + np.floor(diff / 2))

        # Updating the parameters on the slider.
        self._slice_scroll.set_value(new_value)
        self._slice_scroll.reset(maxVal=max_slice, minVal=min_slice)
    ...
    def _click_image(self, event):
        ...
        # If we don't have supervoxels, it does not make sense to change label.
        if self.supervoxel_border is None:
            return
    ...
    def save_segm(self):
        ...
        if self.segm_path is not None:
            ...
            kwargs['initialfile'] = 'corrected_'+basename
    ...
    def open_segm(self):
        ...
        if segm_path is not None and os.path.exists(segm_path):
